...
function load() {
  try {
    ...
    return s && s.cats ? s : defaultState()
...
export default function Proposta({ showToast }) {
  ...
  const toggleCat = id => update(p => ({
    ...p, collapsed: { ...p.collapsed, [id]: !p.collapsed[id] }
  }))

  const setCatName = (id, name) => update(p => ({
    ...p, cats: p.cats.map(c => c.id===id ? {...c, name} : c)
  }))

  const addCat = () => update(p => ({
    ...p, cats: [...p.cats, { id:uid(), name:'', items:[{ id:uid(), desc:'', val:'' }] }]
  }))

  const delCat = id => update(p => ({
    ...p, cats: p.cats.filter(c => c.id!==id)
  }))

  const addItem = catId => update(p => ({
    ...p, cats: p.cats.map(c => c.id===catId
      ? { ...c, items: [...c.items, { id:uid(), desc:'', val:'' }] }
      : c)
  }))
  ...
  const setItemDesc = (catId, itemId, desc) => update(p => ({
    ...p, cats: p.cats.map(c => c.id!==catId ? c : {
      ...c, items: c.items.map(i => i.id===itemId ? {...i, desc} : i)
    })
  }))

  const setItemVal = (catId, itemId, val) => update(p => ({
    ...p, cats: p.cats.map(c => c.id!==catId ? c : {
      ...c, items: c.items.map(i => i.id===itemId ? {...i, val} : i)
    })
  }))
  ...
  return (
    <div style={S.screen}>
      ...
      <div style={S.scroll} className="neo-scroll">
        ...
        <div style={{
          ...S.card,
          borderLeft: `2px solid ${isOver ? '#c07070' : '#4a8fa8'}`,
          marginBottom:16,
        }}>
