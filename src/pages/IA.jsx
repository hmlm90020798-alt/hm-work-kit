...
// ── Groq API ──────────────────────────────────────────────────────────────────
const GROQ_MODELS = [
  { id: 'llama-3.3-70b-versatile', label: 'Llama 3.3 70B',  maxArts: 80,  maxMO: 100 },
  { id: 'llama-3.1-8b-instant',    label: 'Llama 3.1 8B',   maxArts: 40,  maxMO: 60  },
  { id: 'llama3-70b-8192',         label: 'Llama 3 70B',    maxArts: 60,  maxMO: 80  },
]
...
function buildPrompt(descricao, artigos, catsSelBib, seccoesMO, limites) {
  const { maxArts, maxMO } = limites
...
export default function IA({ showToast }) {
  ...
  // ── Estado de modelo — índice do modelo activo ─────────────────────────────
  ...
  useEffect(() => {
    getDocs(collection(db, 'artigos'))
      .then(snap => setArtigos(snap.docs.map(d => ({ id: d.id, ...d.data() }))))
      .catch(() => showToast('Erro ao carregar biblioteca'))
  }, [])
  ...
  // ── Analisar — tenta modelos por ordem a partir do seleccionado ────────────
  const analisar = async () => {
    ...
    // Começa pelo modelo seleccionado, depois faz fallback pelos restantes
    const ordem = [
      ...GROQ_MODELS.slice(modelIdx),
      ...GROQ_MODELS.slice(0, modelIdx),
    ]
    ...
    for (const model of ordem) {
      const prompt = buildPrompt(descricao, artigos, catsSelBib, seccoesMO, { maxArts: model.maxArts, maxMO: model.maxMO })
      try {
        ...
        if (res.status === 404) { lastErr = `${model.label} não disponível`; continue }
        if (res.status === 429) {
          lastErr = `${model.label} — limite atingido, a aguardar 10s…`
          setErro(lastErr); await sleep(10000); setErro(''); continue
        }
        if (res.status === 503) {
          lastErr = `${model.label} — serviço indisponível, a aguardar 5s…`
          setErro(lastErr); await sleep(5000); setErro(''); continue
        }
        if (!res.ok) {
  ...
  const modelActual = GROQ_MODELS[modelIdx]
      ...
      <div className="neo-scroll" style={{ flex: 1, overflowY: 'auto', padding: '16px 14px 32px' }}>
        ...
        {!resultado && !loading && (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {showFiltros && (
                <div style={{ background: 'var(--neo-bg2)', borderRadius: 'var(--neo-radius-sm)', padding: '12px', boxShadow: 'var(--neo-shadow-out-sm)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      <button onClick={() => { setModelDropOpen(o => !o); setBibDropOpen(false); setMoDropOpen(false) }} style={{
                        ...
                        <span>{modelActual.label}</span>
                        <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          <span style={{ fontSize: 7, color: 'var(--neo-text2)' }}>
                            {modelActual.maxArts} arts · {modelActual.maxMO} MO
                          </span>
                          <span style={{ fontSize: 8, opacity: .5 }}>{modelDropOpen ? '▲' : '▼'}</span>
                        </span>
                      </button>
                      {modelDropOpen && (
                        <div className="neo-dropdown" style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: 'var(--neo-bg2)', borderRadius: 'var(--neo-radius-sm)', boxShadow: 'var(--neo-shadow-out)', zIndex: 50 }}>
                          {GROQ_MODELS.map((m, i) => (
                            <button key={m.id} onClick={() => { setModelIdx(i); setModelDropOpen(false) }} style={{
                              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                              width: '100%', padding: '9px 12px',
                              background: modelIdx === i ? 'var(--neo-bg)' : 'transparent',
                              border: 'none', cursor: 'pointer', fontFamily: "'Barlow Condensed'",
                              fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase',
                              color: modelIdx === i ? 'var(--neo-gold)' : 'var(--neo-text2)', textAlign: 'left',
                            }}>
                              <span>{m.label}</span>
                              <span style={{ fontSize: 7, opacity: 0.6 }}>{m.maxArts} arts · {m.maxMO} MO</span>
                            </button>
                          ))}
                        </div>
                      )}
                  ...
                  {/* Linha com dropdowns Biblioteca + MO */}
        ...
        {resultado && !loading && (
          <div>
            {/* Modelo usado */}
            {modelUsado && (
              <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 8, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--neo-text2)', textAlign: 'right', marginBottom: 8, opacity: 0.6 }}>
                via {modelUsado}
              </div>
            )}
